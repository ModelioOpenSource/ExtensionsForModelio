/**
 * Java Class : AddQuantityKindDiagramCommand.java
 *
 * Description :
 *
...
public class QuantityKindDiagramCommand extends DefaultBoxTool {
    ...
    public void actionPerformed(IDiagramHandle representation, IDiagramGraphic target, Rectangle rect) {
        ...
        try( ITransaction transaction = session.createTransaction (I18nMessageService.getString ("Info.Session.Create", "Quantity Kind"))){
            MObject element = target.getElement();
            
            if (element instanceof AbstractDiagram) {
                element = ((AbstractDiagram) element).getOrigin();
            }
            MObject quantityKind = SysMLFactory.createQuantityKind((NameSpace) element);
            List<IDiagramGraphic> graph = representation.unmask(quantityKind, rect.x, rect.y);
