/**
 * Java Class : AddControlOperatorDiagramCommand.java
 *
 * Description :
 *
...
public class ControlOperatorDiagramCommand extends DefaultBoxTool {
    ...
    public void actionPerformed(IDiagramHandle representation, IDiagramGraphic target, Rectangle rect) {
        ...
        try( ITransaction transaction = session.createTransaction (I18nMessageService.getString ("Info.Session.Create" ,"Control Operator"))){
            MObject element = target.getElement();
            MObject operator = SysMLFactory.createControlOperatorOperation((Classifier) element);
            List<IDiagramGraphic> graph = representation.unmask (operator, rect.x, rect.y);
