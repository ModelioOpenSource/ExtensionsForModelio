...
public class TogafFunctionProperty implements IPropertyContent {
	...
	public void changeProperty(ModelElement element, int row, String value) {
		...
		TogafFunction c_element = new TogafFunction((Interface) element);
		if (row == 2) {
			c_element.setvalueFactor(value);
		}
		if (row == 3) {
			c_element.setchangesNeeded(value);
		}
		if (row == 4) {
			c_element.setincrements(value);
		}
		if (row == 5) {
	...
	public void update(ModelElement element, IModulePropertyTable table) {
		...
		TogafFunction c_element = new TogafFunction((Interface) element);
		String value_valueFactor = c_element.getvalueFactor();
		table.addProperty(ResourceManager.getPropertyName("TogafFunction_valueFactor"), value_valueFactor);
		String value_changesNeeded = c_element.getchangesNeeded();
		table.addProperty(ResourceManager.getPropertyName("TogafFunction_changesNeeded"), value_changesNeeded);
		String value_increments = c_element.getincrements();
		table.addProperty(ResourceManager.getPropertyName("TogafFunction_increments"), value_increments);
