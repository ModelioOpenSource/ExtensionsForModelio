...
public class IOFlowProperty implements IPropertyContent {
	...
	public void changeProperty(ModelElement element, int row, String value) {
		try {
			...
			TagType type = TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getMetamodelExtensions().getTagType("TogafArchitect", "IOFlow_habilitation",element.getMClass());
			EList<TaggedValue> tags = element.getTag();
            for (TaggedValue tag : new ArrayList<>(tags)) {
                if (Objects.equals(type, tag.getDefinition())) {
                    tags.remove(tag);
                }
            }
