...
public class ApplicationLayer extends Layer {
	...
	public ApplicationLayer() {
		...
		ModelUtils.setStereotype(this.element,"TogafArchitect", "ApplicationLayer");
		this.element.setName(ResourceManager.getName("ApplicationLayer"));
	}

	public ApplicationLayer(Package element) {
		super(element);
	}
	...
	public List<ApplicationArchitecture> getApplicationArchitecture() {
		List<ApplicationArchitecture> res = new java.util.ArrayList<>();
		for (ModelTree model_element : this.element.getOwnedElement()) {
			res.add(new ApplicationArchitecture((Package) model_element));
		}
		return res;
	}
