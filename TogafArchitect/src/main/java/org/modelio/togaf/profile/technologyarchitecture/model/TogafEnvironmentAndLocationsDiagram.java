...
public class TogafEnvironmentAndLocationsDiagram extends TogafApplicationAndUserLocationDiagram {

    public TogafEnvironmentAndLocationsDiagram(ModelElement owner, IStyleHandle style) throws Exception {
        super(TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getModel().createStaticDiagram(owner.getName() + " " + ResourceManager.getName("TogafEnvironmentAndLocationsDiagram"), owner,
                TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getMetamodelExtensions().getStereotype("TogafArchitect", "TogafEnvironmentAndLocationsDiagram",
        ...
        Note note = session.getModel().createNote(TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getMetamodelExtensions().getNoteType("TogafArchitect", "TOGAFModelingGuide", getElement().getMClass()), getElement(),
                org.modelio.togaf.i18n.Messages.getString("TogafEnvironmentAndLocationsDiagram_NOTE"));
    ...
    public TogafEnvironmentAndLocationsDiagram(StaticDiagram element) {
        super(element);
    }
