...
public class TogafApplicationAndUserLocationDiagram extends TogafPlatformDecompositionDiagram {
    ...
    public TogafApplicationAndUserLocationDiagram(ModelElement owner, IStyleHandle style) throws Exception {
        super(TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getModel().createStaticDiagram(owner.getName() + " " + ResourceManager.getName("TogafApplicationAndUserLocationDiagram"), owner,
                TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getMetamodelExtensions().getStereotype("TogafArchitect", "TogafApplicationAndUserLocationDiagram",
        ...
        Note note = session.getModel().createNote(TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getMetamodelExtensions().getNoteType("TogafArchitect", "TOGAFModelingGuide", getElement().getMClass()), getElement(),
                org.modelio.togaf.i18n.Messages.getString("TogafApplicationAndUserLocationDiagram_NOTE"));
    ...
    public TogafApplicationAndUserLocationDiagram(StaticDiagram element) {
        super(element);
    }
