...
public class TogafOrganizationRoleDiagram extends TogafDiagram {

    public TogafOrganizationRoleDiagram(ModelElement owner, IStyleHandle style) throws Exception {
        super(TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getModel().createStaticDiagram(owner.getName() + " " + ResourceManager.getName("TogafOrganizationRoleDiagram"), owner,
                TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getMetamodelExtensions().getStereotype("TogafArchitect", "TogafOrganizationRoleDiagram",
        ...
        Note note = session.getModel().createNote(TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getMetamodelExtensions().getNoteType("TogafArchitect", "TOGAFModelingGuide", getElement().getMClass()), getElement(),
                org.modelio.togaf.i18n.Messages.getString("TogafOrganizationRoleDiagram_NOTE"));
    ...
    public TogafOrganizationRoleDiagram(StaticDiagram element) {
        super(element);
    }
    ...
    public OrganizationParticipant getOrganizationParticipant() {
        return new OrganizationParticipant((Actor) this.element.getOrigin());
    }
