...
import org.modelio.togaf.profile.applicationarchitecture.model.TogafISServiceOperation;

public class TogafISServiceOperationCommande extends DefaultBoxTool {
	...
	public boolean acceptElement(IDiagramHandle representation, IDiagramGraphic graphic) {
		...
		if (owner.isStereotyped("TogafArchitect","TogafFunction")) {
			return true;
		}
		return false;
	...
	public void actionPerformed(IDiagramHandle representation, IDiagramGraphic graphic, Rectangle rec) {
		...
		try (ITransaction transaction = session.createTransaction("");) {
			...
			TogafISServiceOperation proxy = new TogafISServiceOperation();
