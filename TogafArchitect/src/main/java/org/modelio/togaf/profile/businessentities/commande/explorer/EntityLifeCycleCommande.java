...
public class EntityLifeCycleCommande extends DefaultModuleCommandHandler {
	...
	public boolean accept(List<MObject> selected_element, IModule module) {
		if (selected_element.size() > 0 && selected_element.get(0) instanceof ModelElement) {
			...
			if (element.isStereotyped("TogafArchitect","TogafProduct")) {
				return true;
			}
		}
		return false;
	...
	public void actionPerformed(List<MObject> selected_element, IModule module) {
        ...
        try (ITransaction transaction = session.createTransaction("");) {
			...
			EntityLifeCycle proxy = new EntityLifeCycle();

			if (parent instanceof NameSpace) {
				proxy.setParent((NameSpace) parent);
				if (parent.isStereotyped("TogafArchitect","BusinessEntity")) {
					new TogafDataLifeCycleDiagram(proxy.getElement(),style);
				} else if (parent.isStereotyped("TogafArchitect","TogafProduct")) {
					new TogafProductLifeCycleDiagram(proxy.getElement(),style);
				}
			}
