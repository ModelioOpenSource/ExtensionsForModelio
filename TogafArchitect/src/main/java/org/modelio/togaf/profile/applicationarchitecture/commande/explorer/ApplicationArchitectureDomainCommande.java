...
public class ApplicationArchitectureDomainCommande extends DefaultModuleCommandHandler {
	...
	public boolean accept(List<MObject> selected_element, IModule module) {
		if (selected_element.size() > 0 && selected_element.get(0) instanceof ModelElement) {
			...
			if (element.isStereotyped("TogafArchitect","ApplicationLayer")) {
				return true;
			}
		}
		return false;
	...
	public void actionPerformed(List<MObject> selected_element, IModule module) {
		...
		try (ITransaction transaction = session.createTransaction("");) {
			...
			ApplicationArchitectureDomain proxy = new ApplicationArchitectureDomain();
