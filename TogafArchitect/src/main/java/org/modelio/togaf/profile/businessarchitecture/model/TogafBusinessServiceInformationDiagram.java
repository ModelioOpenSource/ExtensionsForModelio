...
public class TogafBusinessServiceInformationDiagram extends TogafDiagram {
    ...
    public TogafBusinessServiceInformationDiagram(ModelElement owner, IStyleHandle style) throws Exception {
        super(TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getModel().createStaticDiagram(owner.getName() + " " + ResourceManager.getName("TogafBusinessServiceInformationDiagram"), owner,
                TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getMetamodelExtensions().getStereotype("TogafArchitect", "TogafBusinessServiceInformationDiagram",
        ...
        Note note = session.getModel().createNote(TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getMetamodelExtensions().getNoteType("TogafArchitect", "TOGAFModelingGuide", getElement().getMClass()), getElement(),
                org.modelio.togaf.i18n.Messages.getString("TogafBusinessServiceInformationDiagram_NOTE"));
    ...
    public TogafBusinessServiceInformationDiagram(StaticDiagram element) {
        super(element);
    }
