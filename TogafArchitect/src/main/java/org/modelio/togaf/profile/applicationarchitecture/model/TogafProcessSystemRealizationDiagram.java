...
public class TogafProcessSystemRealizationDiagram extends TogafApplicationCommunicationDiagram {

    public TogafProcessSystemRealizationDiagram(ModelElement owner, IStyleHandle style) throws Exception {
        super(TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getModel().createStaticDiagram(owner.getName() + " " + ResourceManager.getName("TogafProcessSystemRealizationDiagram"), owner,
                TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getMetamodelExtensions().getStereotype("TogafArchitect", "TogafProcessSystemRealizationDiagram",
        ...
        Note note = session.getModel().createNote(TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getMetamodelExtensions().getNoteType("TogafArchitect", "TOGAFModelingGuide", getElement().getMClass()), getElement(),
                org.modelio.togaf.i18n.Messages.getString("TogafProcessSystemRealizationDiagram_NOTE"));
    ...
    public TogafProcessSystemRealizationDiagram(StaticDiagram element) {
        super(element);
    }
