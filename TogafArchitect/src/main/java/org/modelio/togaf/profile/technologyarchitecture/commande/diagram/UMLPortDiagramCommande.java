...
public class UMLPortDiagramCommande extends DefaultBoxTool {
	...
	public boolean acceptElement(IDiagramHandle representation, IDiagramGraphic graphic) {
		...
		if (owner instanceof Instance || owner instanceof Classifier) {
			return true;
		}
		return false;
	}
	...
	public void actionPerformed(IDiagramHandle representation, IDiagramGraphic graphic, Rectangle rec) {
		...
		try (ITransaction transaction = session.createTransaction("");) {
			...
			Port proxy = TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getModel().createPort();

			if (parent instanceof Instance) {
				proxy.setCluster((Instance) parent);
			} else if (parent instanceof Classifier) {
				proxy.setInternalOwner((Classifier) parent);
			}

			List<IDiagramGraphic> graph = representation.unmask(proxy, rec.x, rec.y);
			if (graph.size() > 0) {
				((IDiagramNode) graph.get(0)).setBounds(rec);
				((IDiagramNode) graph.get(0)).setProperty("PORT_REPMODE", "STRUCTURED");
			}
