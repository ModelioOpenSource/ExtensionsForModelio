...
import org.modelio.togaf.profile.applicationarchitecture.model.ApplicationArchitectureAttribute;
import org.modelio.vcore.smkernel.mapi.MObject;

public class ApplicationArchitectureAttributeCommande extends DefaultModuleCommandHandler {
	...
	public boolean accept(List<MObject> selected_element, IModule module) {
		if (selected_element.size() > 0 && selected_element.get(0) instanceof ModelElement) {
			...
			if (element.isStereotyped("TogafArchitect","ServiceData")) {
				return true;
			}
			if (element.isStereotyped("TogafArchitect","ServiceDataFragment")) {
				return true;
			}
	...
	public void actionPerformed(List<MObject> selected_element, IModule module) {
		...
		try (ITransaction transaction = session.createTransaction("");) {
			...
			ApplicationArchitectureAttribute proxy = new ApplicationArchitectureAttribute();
