...
public class TogafNewtorkComputingHardwareDiagram extends TogafPlatformDecompositionDiagram {
    ...
    public TogafNewtorkComputingHardwareDiagram(ModelElement owner, IStyleHandle style) throws Exception {
        super(TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getModel().createStaticDiagram(owner.getName() + " " + ResourceManager.getName("TogafNewtorkComputingHardwareDiagram"), owner,
                TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getMetamodelExtensions().getStereotype("TogafArchitect", "TogafNewtorkComputingHardwareDiagram",
        ...
        Note note = session.getModel().createNote(TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getMetamodelExtensions().getNoteType("TogafArchitect", "TOGAFModelingGuide", getElement().getMClass()), getElement(),
                org.modelio.togaf.i18n.Messages.getString("TogafNewtorkComputingHardwareDiagram_NOTE"));
    ...
    public TogafNewtorkComputingHardwareDiagram(StaticDiagram element) {
        super(element);
    }
