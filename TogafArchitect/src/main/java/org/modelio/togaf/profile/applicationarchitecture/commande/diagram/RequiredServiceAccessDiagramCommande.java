...
import org.modelio.togaf.profile.applicationarchitecture.model.RequiredServiceAccess;

public class RequiredServiceAccessDiagramCommande extends DefaultBoxTool {
	...
	public boolean acceptElement(IDiagramHandle representation, IDiagramGraphic graphic) {
		...
		if (owner.isStereotyped("TogafArchitect","TogafApplicationComponent") || owner.isStereotyped("TogafArchitect","TogafApplicationComponentInstance")) {
			return true;
		}
		return false;
	}
	...
	public void actionPerformed(IDiagramHandle representation, IDiagramGraphic graphic, Rectangle rec) {
		...
		try (ITransaction transaction = session.createTransaction("");) {
			...
			RequiredServiceAccess proxy = new RequiredServiceAccess();
