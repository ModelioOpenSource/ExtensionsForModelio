...
public class TogafProductLifeCycleDiagram extends TogafDiagram {

    public TogafProductLifeCycleDiagram(ModelElement owner, IStyleHandle style) throws Exception {
        super(TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getModel().createStateMachineDiagram(owner.getName() + " " + ResourceManager.getName("TogafProductLifeCycleDiagram"), owner,
                TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getMetamodelExtensions().getStereotype("TogafArchitect", "TogafProductLifeCycleDiagram",
        ...
        Note note = session.getModel().createNote(TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getMetamodelExtensions().getNoteType("TogafArchitect", "TOGAFModelingGuide", getElement().getMClass()), getElement(),
                org.modelio.togaf.i18n.Messages.getString("TogafProductLifeCycleDiagram_NOTE"));
    ...
    public TogafProductLifeCycleDiagram(StateMachineDiagram element) {
        super(element);
    }
