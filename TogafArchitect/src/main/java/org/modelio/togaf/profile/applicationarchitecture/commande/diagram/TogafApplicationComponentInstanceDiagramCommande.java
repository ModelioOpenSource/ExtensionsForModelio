...
public class TogafApplicationComponentInstanceDiagramCommande extends DefaultBoxTool {
	...
	public boolean acceptElement(IDiagramHandle representation, IDiagramGraphic graphic) {
		...
		if (owner.isStereotyped("TogafArchitect","TechnologyArchitecture")) {
			return true;
		}
		if (owner.isStereotyped("TogafArchitect","DeploymentServer")) {
			return true;
		}
		if (owner.isStereotyped("TogafArchitect","DeploymentWorkStation")) {
			return true;
		}
		if (owner.isStereotyped("TogafArchitect","TogafApplicationComponent")) {
			return true;
		}

		if (owner.isStereotyped("TogafArchitect","TogafLocation")) {
			return true;
		}
		return false;
	}
	...
	public void actionPerformed(IDiagramHandle representation, IDiagramGraphic graphic, Rectangle rec) {
		...
		try (ITransaction transaction = session.createTransaction("");) {
			...
			TogafApplicationComponentInstance proxy = new TogafApplicationComponentInstance();

			if (parent instanceof Package) {
				proxy.setParent((Package) parent);
			}
			if (parent instanceof Component) {
				proxy.setParent((Component) parent);
			}

			if (parent instanceof Node) {
				proxy.setParent((Node) parent);
			}
