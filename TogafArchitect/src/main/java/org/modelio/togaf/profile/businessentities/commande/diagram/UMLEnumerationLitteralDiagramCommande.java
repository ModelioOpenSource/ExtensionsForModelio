...
public class UMLEnumerationLitteralDiagramCommande extends DefaultBoxTool {
	...
	public boolean acceptElement(IDiagramHandle representation, IDiagramGraphic graphic) {
		...
		if (owner.isStereotyped("TogafArchitect","TogafEnumeration")) {
			return true;
		}
		return false;
	}
	...
	public void actionPerformed(IDiagramHandle representation, IDiagramGraphic graphic, Rectangle rec) {
		...
		try (ITransaction transaction = session.createTransaction("");) {
			...
			EnumerationLiteral proxy = model.createEnumerationLiteral();
			proxy.setName("Literal");

			if (parent instanceof Enumeration) {
				proxy.setValuated((Enumeration) parent);
			}

			representation.unmask(proxy, 0, 0);
			representation.save();
