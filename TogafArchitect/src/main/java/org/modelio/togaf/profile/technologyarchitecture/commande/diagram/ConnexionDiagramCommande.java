...
import org.modelio.togaf.profile.technologyarchitecture.model.Connexion;

public class ConnexionDiagramCommande extends DefaultLinkTool {
	...
	public boolean acceptFirstElement(IDiagramHandle representation, IDiagramGraphic graphic) {
		...
		if (owner.isStereotyped("TogafArchitect","HardwareTechnologyComponent")) {
			return true;
		} else if (owner.isStereotyped("TogafArchitect","TogafApplicationComponentInstance")) {
			return true;
		} else if (owner.isStereotyped("TogafArchitect","TogafBus")) {
			return true;
		} else if (owner instanceof Port) {
			return true;
		} else if (owner instanceof Instance) {
			return true;
		} else if (owner.isStereotyped("TogafArchitect","TogafLocation")) {
			return true;
		}
		return false;
	}
	...
	public void actionPerformed(IDiagramHandle representation, IDiagramGraphic graphic_source, IDiagramGraphic graphic_target, LinkRouterKind kind, ILinkPath path) {
		...
		try (ITransaction transaction = session.createTransaction("");) {
			...
			Connexion proxy = new Connexion(source, target);
