...
public class TogafServiceContractProperty implements IPropertyContent {
	...
	public void changeProperty(ModelElement element, int row, String value) {
		...
		if (row == 2) {
			try {
				element.putNoteContent("TogafArchitect", "Measure", value);
			} catch (Exception e ) {
				e.printStackTrace();
			}
		}
	...
	public void update(ModelElement element, IModulePropertyTable table) {
		...
		String contract = element.getNoteContent(",.*","Measure");
		if (contract == null) {
			contract = "";
		}
		table.addProperty(ResourceManager.getPropertyName("Measure"), contract);
	}
