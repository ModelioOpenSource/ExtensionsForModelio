...
public class TogafFunctionalDecompositionDiagram {
    protected StaticDiagram element;

    public TogafFunctionalDecompositionDiagram(ModelElement owner, IStyleHandle style) {
        try {
            this.element = TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getModel().createStaticDiagram(owner.getName() + " " + ResourceManager.getName("TogafFunctionalDecompositionDiagram"), owner,
                    TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getMetamodelExtensions().getStereotype("TogafArchitect", "TogafFunctionalDecompositionDiagram",
            ...
            Note note = session.getModel().createNote(TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getMetamodelExtensions().getNoteType("TogafArchitect", "TOGAFModelingGuide", getElement().getMClass()), getElement(),
                    org.modelio.togaf.i18n.Messages.getString("TogafFunctionalDecompositionDiagram_NOTE"));
    ...
    public TogafFunctionalDecompositionDiagram(StaticDiagram element) {
