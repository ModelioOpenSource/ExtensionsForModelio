...
import org.modelio.togaf.profile.businessarchitecture.model.TogafConsumes;

public class TogafConsumesDiagramCommande extends DefaultLinkTool {
	...
	public boolean acceptSecondElement(IDiagramHandle representation, IDiagramGraphic graphic_source, IDiagramGraphic graphic_target) {
		...
		if (owner.isStereotyped("TogafArchitect","TogafServiceOperation")) {
			return true;
		}
	...
	public void actionPerformed(IDiagramHandle representation, IDiagramGraphic graphic_source, IDiagramGraphic graphic_target, LinkRouterKind kind, ILinkRoute path) {
		...
		try (ITransaction transaction = session.createTransaction("");) {
			...
			TogafConsumes proxy = new TogafConsumes();
