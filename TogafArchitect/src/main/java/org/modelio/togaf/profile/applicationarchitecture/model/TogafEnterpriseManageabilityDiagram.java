...
public class TogafEnterpriseManageabilityDiagram extends TogafApplicationCommunicationDiagram {

    public TogafEnterpriseManageabilityDiagram(ModelElement owner, IStyleHandle style) throws Exception {
        super(TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getModel().createStaticDiagram(owner.getName() + " " + ResourceManager.getName("TogafEnterpriseManageabilityDiagram"), owner,
                TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getMetamodelExtensions().getStereotype("TogafArchitect", "TogafEnterpriseManageabilityDiagram",
        ...
        Note note = session.getModel().createNote(TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getMetamodelExtensions().getNoteType("TogafArchitect", "TOGAFModelingGuide", getElement().getMClass()), getElement(),
                org.modelio.togaf.i18n.Messages.getString("TogafEnterpriseManageabilityDiagram_NOTE"));
    ...
    public TogafEnterpriseManageabilityDiagram(StaticDiagram element) {
        super(element);
    }
