...
public class TogafOrganizationDecompositionDiagram extends TogafDiagram {

    public TogafOrganizationDecompositionDiagram(ModelElement owner, IStyleHandle style) throws Exception {
        super(TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getModel().createStaticDiagram(owner.getName() + " " + ResourceManager.getName("TogafOrganizationDecompositionDiagram"), owner,
                TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getMetamodelExtensions().getStereotype("TogafArchitect", "TogafOrganizationDecompositionDiagram",
        ...
        Note note = session.getModel().createNote(TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getMetamodelExtensions().getNoteType("TogafArchitect", "TOGAFModelingGuide", getElement().getMClass()), getElement(),
                org.modelio.togaf.i18n.Messages.getString("TogafOrganizationDecompositionDiagram_NOTE"));
    ...
    public TogafOrganizationDecompositionDiagram(StaticDiagram element) {
        super(element);
    }
