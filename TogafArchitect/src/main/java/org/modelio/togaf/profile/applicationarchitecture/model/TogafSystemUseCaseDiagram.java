...
public class TogafSystemUseCaseDiagram extends TogafUseCaseDiagram {

    public TogafSystemUseCaseDiagram(ModelElement owner, IStyleHandle style) throws Exception {
        super(owner, "TogafArchitect", "TogafSystemUseCaseDiagram", style);
        ...
        Note note = session.getModel().createNote(TogafArchitectModule.getInstance().getModuleContext().getModelingSession().getMetamodelExtensions().getNoteType("TogafArchitect", "TOGAFModelingGuide", getElement().getMClass()), getElement(),
                org.modelio.togaf.i18n.Messages.getString("TogafSystemUseCaseDiagram_NOTE"));
