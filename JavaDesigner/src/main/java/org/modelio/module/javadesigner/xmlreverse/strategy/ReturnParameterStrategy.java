...
public class ReturnParameterStrategy extends ModelElementStrategy implements IReverseBox<JaxbReturnParameter,Parameter> {
    
    public ReturnParameterStrategy(final IModelingSession session) {
        super(session);
    }

    
    @Override
    public Parameter getCorrespondingElement(final JaxbReturnParameter jaxb_element, final MObject owner, final IReadOnlyRepository repository) {
        if(owner instanceof Operation) {
            Operation owner_operation = (Operation) owner;    
            Parameter parameter = owner_operation.getReturn();
            if(parameter != null ){            
                return parameter;
            }
        }
    ...
    public List<MObject> updateProperties(final JaxbReturnParameter jaxb_element, final Parameter modelio_element, final MObject owner, final IReadOnlyRepository repository) {
        if(owner instanceof Operation){
            modelio_element.setReturned((Operation)owner);
        }else{
    ...
    @Override
    public void deleteSubElements(final JaxbReturnParameter jaxb_element, final Parameter modelio_element, final Collection<MObject> element_todelete, final IReadOnlyRepository repository) {
