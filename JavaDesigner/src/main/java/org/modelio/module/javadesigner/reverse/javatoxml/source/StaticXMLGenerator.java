...
class StaticXMLGenerator extends XMLGenerator {
    ...
    public void generateXML(final ASTTree ast, final Context ctx) throws BadNodeTypeException, IOException, XMLGeneratorException {
        if (ast.getType () != JavaParser.STATIC_INIT) {
            throw new BadNodeTypeException (Messages.getString ("reverse.Node_must_be_STATIC_INIT"), ast); //$NON-NLS-1$
        }
        ...
        //                StringBuilder body = GeneratorUtils.getBodyCode (child);
        //                content.append ("static {" + body + "}\n"); //$NON-NLS-1$ //$NON-NLS-2$
